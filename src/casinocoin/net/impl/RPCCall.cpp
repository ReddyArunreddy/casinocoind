...
private:
    ...
    static Json::Value jvParseCurrencyIssuer (std::string const& strCurrencyIssuer)
    {
        ...
        if (boost::regex_match (strCurrencyIssuer, smMatch, reCurIss))
        {
            ...
            if (strIssuer.length ())
            {
                // Could confirm issuer is a valid casinocoin address.
...
private:
    ...
    bool isValidJson2(Json::Value const& jv)
    {
        if (jv.isObject())
        {
            if (jv.isMember(jss::jsonrpc) && jv[jss::jsonrpc] == "2.0" &&
                jv.isMember(jss::casinocoinrpc) && jv[jss::casinocoinrpc] == "2.0" &&
    ...
    Json::Value parseJson2(Json::Value const& jvParams)
    {
        ...
        if (valid_parse && isValidJson2(jv))
        {
            ...
            jv1[jss::casinocoinrpc] = jv[jss::casinocoinrpc];
        ...
        if (jv.isMember(jss::casinocoinrpc))
            jv_error[jss::casinocoinrpc] = jv[jss::casinocoinrpc];
...
cmdLineToJSONRPC (std::vector<std::string> const& args, beast::Journal j)
{
    ...
    if (paramsObj.isMember(jss::casinocoinrpc))
        jv[jss::casinocoinrpc] = paramsObj[jss::casinocoinrpc];
    ...
    Config const& config, Logs& logs)
{
    ...
    if (jvRequest.isMember(jss::casinocoinrpc))
        jvOutput[jss::casinocoinrpc] = jvRequest[jss::casinocoinrpc];
