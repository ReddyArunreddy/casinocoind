...
#include <casinocoin/server/impl/JSONRPCUtil.h>
#include <casinocoin/rpc/impl/ServerHandlerImp.h>
            ...
            Json::Value const& jv)
{
         ...
         jv[jss::command].asString() != jv[jss::method].asString()))
    {
        ...
        if (jv.isMember(jss::casinocoinrpc))
            jr[jss::casinocoinrpc] = jv[jss::casinocoinrpc];
        ...
        std::string forwardedFor, std::string user)
{
    ...
    if (jsonRPC.isMember(jss::casinocoinrpc))
        reply[jss::casinocoinrpc] = jsonRPC[jss::casinocoinrpc];
    ...
    http_request_type const& request) const
{
    ...
    if (app_.serverOkay(reason))
    {
        ...
        msg.body = "<!DOCTYPE html><html><head><title>" + systemName() +
            " Test page for casinocoind</title></head><body><h1>" +
                systemName() + " Test</h1><p>This page shows casinocoind http(s) "
